<?php
    ...
    class CategoryColor
    {
        ...
        public function __construct($key)
        {
    		...
    		add_action( 'category_add_form_fields', array($this, 'categoryFormFieldsAction'), 10, 2 );

    		// Add the field to the Edit Category page
    		add_action( 'category_edit_form_fields',  array($this, 'categoryFormFieldsAction'), 10, 2 );

    		// Save extra taxonomy fields callback function.
    		add_action( 'edited_category', array($this, 'saveCategoryColorAction'), 10, 2 );
    		add_action( 'create_category', array($this, 'saveCategoryColorAction'), 10, 2 );
        ...
        public function saveCategoryColorAction($term_id)
    	{
    		if ( !isset( $_POST['term_meta'] ) )
    			return false;

    	    $name = $this->key . '_' . $term_id;
        ...
        public function categoryFormFieldsAction($term)
    	{
    		$term_value = '';

    		if( is_object($term) && $term->term_id ) {
    			$term_meta = $this->key . '_' . $term->term_id;
        ...
        public function addColorColumnOnCategoryPage()
        {
            ...
            add_filter('manage_edit-category_columns', function($columns) {
                $columns[$this->key . '_column'] = __('Color');
                return $columns;
            });
            ...
            add_filter ('manage_category_custom_column', function($content, $column_name, $term_id) {
                ...
                if($column_name == $this->key . '_column') {
        ...
        public function addColorInWPTermObject()
        {
        ...
        {
            $cat_data = get_option($this->key . '_' . $categoryId);
